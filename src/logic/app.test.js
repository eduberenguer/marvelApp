...
test("Retrieve characters(40)", () => {
  return logic.retrieveCharacters().then(res => {
...
test("Retrieve characters with custom limit(60)", () => {
  return logic.retrieveCharacters(60).then(res => {
    expect(res.data.limit).toEqual(60);
  });
});

test("Retrieve characters by Id", () => {
  return logic.retriveCharacterById(1010354).then(res => {
...
test("Retrieve characters by fail", () => {
  return logic.retriveCharacterById(123456).then(res => {
    expect(res.data).toBeUndefined();
  });
});

test("Filter with appearances comics type", () => {
  return logic.filterWithAppearances(30, "comics").then(res => {
...
test("Filter with appearances series type", () => {
  return logic.filterWithAppearances(10, "series").then(res => {
...
test("Filter with appearances stories type", () => {
  return logic.filterWithAppearances(50, "stories").then(res => {
...
test("Retrieve character with name", () => {
  return logic.retrieveCharacterByName("Thor").then(res => {
...
test("Retrive data with name start by...", () => {
  return logic.nameStartWith("Ma").then(res => {
    console.log(res.data.results);
...
test("Retrive data with name start by...fail", () => {
  return logic.nameStartWith("asdasdasdsad").then(res => {
    console.log(res.data.results);
